use crate::{
  bigint::BigInt,
  utils::{
    digital_add, digital_decrement, digital_increment, digital_subtract,
    DigitalWrap, IncrementWrap,
  },
};
...
#[test]
fn test_digital_increment() {
  let mut digits = vec![0];
  for _ in 0..1000 {
    digital_increment(&mut digits, IncrementWrap::Ten);
    println!("{:?}", digits);
  }
}

#[test]
fn test_digital_decrement() {
  let mut digits = vec![0, 0, 0, 1];
  for _ in 0..1001 {
    digital_decrement(&mut digits, 10);
    println!("{:?}", digits);
  }
}
